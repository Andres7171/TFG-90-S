  ...
  return (
    <AdminLayout>
      <div className="admin-page-header">
        <h2 className="admin-page-title">90'S Type Shit — Productos Propios</h2>
        <div className="d-flex gap-2 align-items-center">
          ...
          />
          <button className="admin-add-btn" onClick={() => navigate('/admin/90s/nuevo')}>
            + Añadir
          </button>
        </div>
      </div>
        ...
        <div className="admin-table-wrap">
            ...
            <tbody>
              {filtered.map((p) => (
                <tr key={p.id} className={`admin-table-row${p.brand?.active === false ? ' brand-inactive' : ''}`}>
                  <td>
                    <img
                      src={p.image_url || 'https://placehold.co/52x52/1a1a1a/FFD700?text=90S'}
                      alt={p.name}
                      className="admin-table-img"
                      onError={(e) => { e.target.src = 'https://placehold.co/52x52/1a1a1a/FFD700?text=90S' }}
                    />
                  </td>
                  ...
                  <td>
                    <div className="d-flex gap-2">
                      <button
                        ...
                        onClick={() => navigate(`/admin/90s/${p.id}`)}
                      >
                        Editar
                      </button>
                      <button
          ...
          {filtered.length === 0 && (
            <p style={{ color: '#555', textAlign: 'center', padding: '2rem', fontSize: '0.85rem' }}>
              No se encontraron productos propios.
            </p>
          )}
        </div>
      )}
